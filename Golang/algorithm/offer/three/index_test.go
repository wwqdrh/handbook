...
func TestHand22(t *testing.T) {
	res := Hand22(list.NewListNode([]int{1, 2, 3, 4, 5}), 2).ToSlice()
	if res[0] != 4 || res[1] != 5 {
		t.Error("发生错误")
	}
}
...
func TestHand24(t *testing.T) {
	res := Hand24(list.NewListNode([]int{1, 2, 3})).ToSlice()
...
func TestHand25(t *testing.T) {
	if !utils.IntSliceCompare(
		Hand25(list.NewListNode([]int{1, 2, 4}), list.NewListNode([]int{1, 3, 4})).ToSlice(),
		[]int{1, 1, 2, 3, 4, 4},
	) {
		t.Error("发生错误")
	}
...
func TestHand26(t *testing.T) {
	if Hand26(tree.NewTreeNode([]string{"1", "2", "3"}), tree.NewTreeNode([]string{"3", "1"})) {
		t.Error("发生错误")
	}
}

func TestHand27(t *testing.T) {
	if Hand27(tree.NewTreeNode([]string{"4", "2", "7", "1", "3", "6", "9"})).ToString() != "4,7,2,9,6,3,1" {
		t.Error("发生错误")
	}
}

func TestHand28(t *testing.T) {
	if !Hand28(tree.NewTreeNode([]string{"1", "2", "2", "3", "4", "4", "3"})) {
		t.Error("发生错误")
	}
	if Hand28(tree.NewTreeNode([]string{"1", "2", "2", "null", "3", "null", "3"})) {
		t.Error("发生错误")
	}
}
...
func TestHand32_1(t *testing.T) {
	if !utils.IntSliceCompare(
		Hand32_1(tree.NewTreeNode([]string{"3", "9", "20", "null", "null", "15", "7"})),
		[]int{3, 9, 20, 15, 7},
	) {
		t.Error("发生错误")
	}
