import grpc
from micro_service.grpc.proto import routeguide_db, routeguide_pb2, routeguide_pb2_grpc
import random
...
def get_feature(feature):
    ...
    if feature.name:
        print(
            "Feature called {name} at {location}".format(
...
def generate_route(feature_list):
    for _ in range(0, 20):
        ...
        print(
            "random feature {name} at {location}".format(
...
def generate_route_note():
    ...
    for msg in msgs:
        print(
            "send message {message} location {location}".format(
...
def run():
    ...
    for msg in response:
        print(
            "recived message {message} location {location}".format(
                message=msg.message, location=msg.location
            )
        )
