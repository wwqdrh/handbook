...
ShutdownCallback: T.TypeAlias = (
    T.Awaitable[T.Any] |
    T.Callable[[asyncio.AbstractEventLoop], T.Awaitable[T.Any]] |
    T.Callable[[asyncio.AbstractEventLoop], None]
)

logger = logging.getLogger("aiorun")
WINDOWS = sys.platform == "win32"
_DO_NOT_CANCEL_COROS: WeakSet[T.Coroutine[None, None, T.Any]] = WeakSet()


def shutdown_waits_for(coro: T.Coroutine[None, None, T.Any], loop: asyncio.AbstractEventLoop | None = None):
...
def run(
    coro: None | T.Coroutine[None, None, T.Any] = None,
    *,
